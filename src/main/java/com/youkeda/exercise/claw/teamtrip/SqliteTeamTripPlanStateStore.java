 ...
 *
 * 数据结构：
 * - 表: team_trip_plans，主键是 user_id
 * - 使用 INSERT OR REPLACE 实现 upsert
 */
@Component
@ConditionalOnProperty(name = "storage.enabled", havingValue = "true")
public class SqliteTeamTripPlanStateStore implements TeamTripPlanStateStore {

    private static final Logger log = LoggerFactory.getLogger(SqliteTeamTripPlanStateStore.class);

    private final JdbcTemplate jdbc;
    private final ObjectMapper objectMapper;
    private final StorageProperties properties;

    public SqliteTeamTripPlanStateStore(JdbcTemplate jdbc, ObjectMapper objectMapper,
    ...
    public TeamTripPlanDraft get(String userId) {
        String sql = "SELECT plan_json FROM team_trip_plans WHERE user_id = ?";
        try {
    ...
    public void save(String userId, TeamTripPlanDraft draft) {
        try {
            ...
            // 使用 INSERT OR REPLACE 实现 upsert
            jdbc.update("""
                INSERT OR REPLACE INTO team_trip_plans (user_id, plan_json, updated_at)
                VALUES (?, ?, ?)
            """, userId, json, now);
    ...
    public void clear(String userId) {
        jdbc.update("DELETE FROM team_trip_plans WHERE user_id = ?", userId);
    }
}
