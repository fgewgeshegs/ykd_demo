...
class ScheduleReplyGuardTest {
    ...
    void injectsHintWhenModelClaimsCreatedButToolNotCalled() {
        ...
        String hint = guard.inspectBeforeReply(
                "帮我设置提醒", "已为你创建好了明天8点的提醒", new HashSet<>());
        assertNotNull(hint, "声称已创建但未调用工具时应注入提示");
    }
    ...
    void allowsClarifyingQuestion() {
        ...
        String hint = guard.inspectBeforeReply(
                "帮我设置提醒", "好的，请问几点提醒你呢？", new HashSet<>());
        assertNull(hint, "向用户澄清时应放行，避免死循环");
    }
    ...
    void allowsNonCreateIntent() {
        ...
        String hint = guard.inspectBeforeReply(
                "我有哪些提醒", "你有3条提醒", new HashSet<>());
        assertNull(hint, "非创建意图不应触发 guard");
    }
    ...
    void allowsWhenToolWasActuallyCalled() {
        ...
        String hint = guard.inspectBeforeReply(
                "帮我设置提醒", "已为你创建好了喝水提醒", executed);
        assertNull(hint, "已实际调用创建工具时应放行");
    }
}
